  ...
  >
    <div>
      <NuxtLink to="/" class="text-3xl">
        <span class="text-red-500 font-bold">MEGA</span>.news
      </NuxtLink>
    </div>
    <div class="flex w-full gap-2 lg:w-fit">
      ...
      >
        <button class="pb-1" title="Toggle dots.">
      ...
      <button
        title="Bookmarks"
        class="bg-gray-100 rounded-lg w-fit p-2 mr-auto"
      >
...
const getLanguages = async () => {
  const languages = await useFetch(
    `${runtimeConfig.public.NEWS_API_URL}/available/languages`
  );
  const rawLanguages = languages.data.value.languages;
